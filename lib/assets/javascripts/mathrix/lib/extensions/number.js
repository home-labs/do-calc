(function ($) {
    ...
    try {
        Object.defineProperties($.prototype, {
            multipleOf: {
                value: function (numbers) {
                    ...
                    args.forEach(function(item, i) {
                        args[i] = parseFloat(Math.abs(arguments[i]));
                    });
                    ...
                    i = 0;
                    while(true) {
                        if(n > args[i] || args[i] % n !== 0) {return false;}
            ...
            // consertar. Se o maior não vier no início, não funciona
            areMultiples: {
                value: function (numbers) {
                    var
                        i = 0,
                        current = parseFloat(this),
                        args = Array.from(arguments).uniq().desc();

                    while(true) {
                        while(true) {
                            if(!args[i++].multipleOf(current)) {return false;}
